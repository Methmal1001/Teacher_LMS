    ...
    <div>
        <form class="space-y-4">
          ...
          <div class="flex items-center gap-2 mt-2">
            ...
            <label for="subscr" class="text-sm text-gray-700">Enable subscription for this course's tasks</label>
          </div>

          <!-- Actions -->
          <div class="flex flex-col sm:flex-row justify-end gap-4 pt-4">
            <button type="button" @click="$emit('close')" class="px-6 py-3 bg-gray-200 rounded-lg hover:bg-gray-300">Cancel</button>
            <button type="button" @click="saveCourse" :disabled="saving" class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-60">
        ...
        <AssignmentUpdate
          v-if="selectedAssignmentForEdit"
          :assignment-id="selectedAssignmentForEdit.id"
          :assignment-name="selectedAssignmentForEdit.name"
          :assignment-due="selectedAssignmentForEdit.dueDate"
          @close="selectedAssignmentForEdit = null"
          @update="updateAssignmentInList"
        />
        
        <!-- Modal for Editing Notes -->
        <UpdateNotes
          v-if="selectedNoteforEdit"
          :note-id="selectedNoteforEdit.id"
          :note-name="selectedNoteforEdit.name"
          @close="selectedNoteforEdit = null"
          @update="updateNoteInList"
        />

        <!-- Modal for Editing Vedios -->
        <UpdateVedios
          v-if="selectedVedioforEdit"
          :vedio-id="selectedVedioforEdit.id"
          :vedio-name="selectedVedioforEdit.name"
          @close="selectedVedioforEdit = null"
          @update="updateVedioInList"
        />

        <!-- Modal for Editing Papers -->
        <UpdatePapers
          v-if="selectedPaperforEdit"
          :paper-id="selectedPaperforEdit.id"
          :paper-name="selectedPaperforEdit.name"
          @close="selectedPaperforEdit = null"
