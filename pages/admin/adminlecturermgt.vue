  ...
  <div class="min-h-screen bg-gray-100 p-6 sm:p-10 ">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
      ...
      <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        <input v-model="searchQuery" type="text" placeholder="Search by lecturer code or name..."
          class="px-4 py-2 border rounded-lg w-full sm:w-72 focus:ring-2 focus:ring-blue-400" />
        <button class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700" @click="isAddLecturer">
          + Add New Lecturer
        </button>
      </div>
    </div>
    ...
    <div class="overflow-x-auto bg-white rounded-xl shadow-md">
      <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
        <table class="w-full text-sm text-left rtl:text-right text-body">
          ...
          <tbody>
            <tr v-for="lecturer in lecturers" :key="lecturer" class="bg-neutral-primary-soft border-b border-default hover:bg-neutral-secondary-medium">
              <td class="w-4 p-4">
                <div class="flex items-center">
              ...
              <td class="px-6 py-4">
                <div class="flex">
                  ...
                  <a href="#" class="font-medium text-fg-brand hover:underline p-2 bg-red-200">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
                      <path
    ...
    <transition name="modal">
      <div v-if="selectedLecturer" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"
        ...
        <div class="relative bg-white w-full max-w-6xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl">
          ...
          <div class="sticky top-0 bg-white border-b p-4 flex justify-between items-center z-10">
            <h2 class="text-xl font-semibold text-gray-800">
              {{ activeModal === 'lecturerAdd' ? 'Add New Lecturer' : activeModal === 'lecturerView' ? 'View Lecturer' : activeModal === 'lecturerEdit' ? 'Edit Lecturer' : '' }}
            </h2>
          ...
          <div class="p-6">
            <!-- ================= ADD Lecturer MODAL ================= -->
            <AdminLecturerAdd v-if="activeModal === 'lecturerAdd'" @close="closeModal" />
            <!-- ================= VIEW Lecturer MODAL ================= -->
            <AdminLecturerView v-if="activeModal === 'lecturerView'" @close="closeModal" />
            <!-- ================= EDIT Lecturer MODAL ================= -->
            <AdminLecturerEdit v-if="activeModal === 'lecturerEdit'" @close="closeModal" />
          </div>
        </div>
      </div>
    </transition>
...
<script> 
import AdminLecturerView from '~/components/admin/lecturer/AdminLecturerView.vue'
import AdminLecturerEdit from '~/components/admin/lecturer/AdminLecturerEdit.vue'
import AdminLecturerAdd from '~/components/admin/lecturer/AdminLecturerAdd.vue'
...
export default {
  ...
  methods: {
    isAddLecturer(lecturer) {
      this.selectedLecturer = lecturer
      this.activeModal = 'lecturerAdd'
      document.body.classList.add('overflow-hidden')
      window.addEventListener('keydown', this.handleEsc)
    },
    isViewLecturer(lecturer) {
      this.selectedLecturer = lecturer
      this.activeModal = 'lecturerView'
      document.body.classList.add('overflow-hidden')
      window.addEventListener('keydown', this.handleEsc)
    },
    isEditLecturer(lecturer) {
      this.selectedLecturer = lecturer
      this.activeModal = 'lecturerEdit'
      document.body.classList.add('overflow-hidden')
      window.addEventListener('keydown', this.handleEsc)
    },
